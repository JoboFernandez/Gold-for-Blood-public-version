...
class Collectible:
    ...
    def __init__(self, x: float, y: float, images: List[dict], scale: Tuple[int, int] = None, name=""):
        self.x = x
        self.y = y
        self.dx = 0
        self.dy = 0
        ...
        default_images = [image for image in images]
        self.animations = {
            "default": {
                "images": load_images(default_images, scale=scale),
                "thresholds": [5 * (i + 1) for i in range(len(default_images))],
